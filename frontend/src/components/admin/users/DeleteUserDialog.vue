...
<template>
  <ResponsiveDialog :open="props.open" @update:open="emit('update:open', $event)">
    <ResponsiveDialogContent>
      <ResponsiveDialogHeader>
        <ResponsiveDialogTitle>{{ t('admin.users.deleteDialogTitle') }}</ResponsiveDialogTitle>
        <ResponsiveDialogDescription>
          {{
            needsTransfer
      ...
      <ResponsiveDialogBody class="pb-2">
        <div v-if="ownedLoading" class="flex items-center gap-2 py-2 text-sm text-muted-foreground">
