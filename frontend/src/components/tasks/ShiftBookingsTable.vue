...
<template>
  <div>
    ...
    <p v-else-if="bookings.length === 0" class="text-sm text-muted-foreground py-2">
      {{ t('duties.shifts.detail.noBookings') }}
    </p>
    <div v-else class="rounded-md border overflow-x-auto">
      <Table class="table-fixed w-full">
        <TableHeader>
          <TableRow>
            <TableHead class="w-[45%]">{{ t('duties.shifts.detail.userName') }}</TableHead>
            <TableHead v-if="authStore.isAdmin" class="w-[25%]">
              {{ t('duties.shifts.detail.userNotes') }}
            </TableHead>
            <TableHead class="w-[30%] text-right">
              {{ t('duties.shifts.detail.bookedAt') }}
            </TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
