...
class TestEventManagerEndpoints:
    ...
    async def test_assign_manager_as_admin(
        ...
        data = r.json()
        assert data["id"] == str(test_task_manager_user.id)
        assert data["email"] == test_task_manager_user.email
    ...
    async def test_assigned_manager_appears_in_list(
        ...
        assert str(test_task_manager_user.id) in ids
    ...
    async def test_removed_manager_disappears_from_list(
        ...
        assert r.status_code == 200
        ids = [m["id"] for m in r.json()]
        assert str(test_task_manager_user.id) not in ids
