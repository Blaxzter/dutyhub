...
class SSEManager:
    ...
    def __init__(self) -> None:
        self._connections: dict[uuid.UUID, set[asyncio.Queue[dict[str, Any]]]] = defaultdict(set)
    ...
    def connect(self, user_id: uuid.UUID) -> asyncio.Queue[dict[str, Any]]:
        ...
        logger.debug(f"SSE connect: user={user_id}, total={len(self._connections[user_id])}")
        return queue

    def disconnect(self, user_id: uuid.UUID, queue: asyncio.Queue[dict[str, Any]]) -> None:
    ...
    async def broadcast(self, user_id: uuid.UUID, event: str, data: dict[str, Any]) -> None:
